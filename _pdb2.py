...
def getCenterandTV(atomGroup, atomGroup2, ftResults, rotations, center=None):
    # orig_coords = atomGroup.getCoords()
    if center is None:
        lig_coords = atomGroup2.getCoords()
...
def transformInputs(rotationStream, entry, masterLig, ligName, origLig):
    master_lig_pdb = parsePDB(masterLig)
    orig_pdb = parsePDB(origLig)
    centers, postTV = getCenterandTV(master_lig_pdb, orig_pdb, entry, rotationStream)
    TTT = getTTT(entry, rotationStream, centers, postTV)
    target = cmd.get_object_list("{}".format(ligName))
    return target, TTT


def genPDBinPymol(rotationFile, ftentry, masterLig, name, origLig):
    rotationStream = readRotations(rotationFile)
    lig, matrix = transformInputs(rotationStream, ftentry, masterLig, name, origLig)
