...
def idInputs(jobFolder, ligCrys=None):
    ...
    if os.path.exists(jobFolder) is True:
        rec = glob(os.path.join(jobFolder, 'rec.pdb.gz'))
        ...
        if ligCrys is None:
            ...
            inputs = lig + ft + rotprm + rec + ligCrys
            return inputs
...
def getInputs(job):
    rotprm = glob(os.path.join(job, 'rot70k.*'))
    lig = glob(os.path.join(job, 'lig.pdb'))
    ft000 = glob(os.path.join(job, 'ft.000.*'))
    ft002 = glob(os.path.join(job, 'ft.002.*'))
    ft004 = glob(os.path.join(job, 'ft.004.*'))
    ft006 = glob(os.path.join(job, 'ft.006.*'))
    rec = glob(os.path.join(job, 'rec.pdb'))
    inputs = ft000 + ft002 + ft004 + ft006 + lig + rec + rotprm
    return inputs
