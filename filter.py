...
def vdwClash(atom1, atom2, dist, threshold):
    ...
    if atomone == 'H': atom1 = h_vdw
    if atomone == 'C': 
        if atom1[0:1]=='Cl': atom1 = cl_vdw    
        else: atom1 = c_vdw       
    if atomone == 'N': atom1 = n_vdw
    if atomone == 'O': atom1 = o_vdw
    if atomone == 'F': atom1 = g_vdw
    if atomone == 'P': atom1 = p_vdw
    if atomone == 'S': atom1 = s_vdw
    
    if atomtwo == 'H': atom2= h_vdw
    if atomtwo == 'C': 
        if atom2[0:1]=='Cl': atom2 = cl_vdw    
        else: atom2 = c_vdw   
    if atomtwo == 'N': atom2 = n_vdw
    if atomtwo == 'O': atom2 = o_vdw
    if atomtwo == 'F': atom2 = g_vdw
    if atomtwo == 'P': atom2 = p_vdw
    if atomtwo == 'S': atom2 = s_vdw
...
def reportFilter(rotationFile, ftFile, alignMap, masterLig, pwPathA, pwPathB):
    ...
    for entry in ftStream:
        ...
        pairwise_dist("mol2", "lig_interface1", "4", output_path=output_path1, output="P", sidechain="Y")
        pairwise_dist("mol1", "lig_interface2", "4", output_path=output_path2, output="P", sidechain="Y")
        if doWeClash(output_path1) > percent_threshold:
            ...
            print ("{} was not clashing with a tolerence of {} percent".format(count, threshold, percent_threshold))
            if dst < 20.0:
                cmd.delete("{}".format(MOL2))
